...
def poly_consolidate(poly):
    variables = AutoVivification()
    for coeff, power, var in poly:
        variables[var][power] = variables[var].get(power, 0) + coeff
    conspoly = []
    for var, sub in variables.items():
        for power, coeff in sub.items():
            conspoly.append([coeff, power, var])
    return conspoly
